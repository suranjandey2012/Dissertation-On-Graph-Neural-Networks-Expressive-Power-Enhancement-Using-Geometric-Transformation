...
class Custom_GNN(nn.Module):
    def __init__(self,L,input_dim,hidden_dim,out_dim,dropout,device,flag,readout='add',dataset_name='',act_fn=nn.SiLU(),emb_dim1=0,emb_dim2=0):
        ...
        self.w_feat=nn.Linear(1,1,device=self.device,dtype=torch.float32)
        self.w_coord=nn.Linear(1,1,device=self.device,dtype=torch.float32)
    ...
    def Compute_Angular_Distance(self,features:torch.tensor,x:torch.tensor) -> tuple:
        ...
        theta_f = torch.asin(torch.clamp(features_angle, -0.99, 0.99))
        theta_x = torch.asin(torch.clamp(x_angle, -0.99, 0.99))
        
        return theta_f, theta_x


    #Method that is used to compute the message vectors between 2 neighbours
    def Compute_Message_Vector(self,A:torch.tensor,features:torch.sparse.Tensor,x:torch.tensor,theta_f:torch.tensor,theta_x:torch.tensor) -> torch.tensor:
        edges=A.coalesce().indices() #Fetch the indices first from the Adjacency matrix
        
        H_i=features[edges[0]]  #Features of the starting vertices of the edges
        
        H_j=features[edges[1]]  #Features of the ending vertices of the edges
        
        Theta_ij=theta_f[edges[0],edges[1]] #Extract theta_f(i,j)
        

        M1=(H_i-H_j)*self.w_feat(Theta_ij.unsqueeze(-1)) #(h_i-h_j)*theta_f(i,j)
        

        X_i=x[edges[0]] #Coordinates of the starting vertices of the edges

        x_j=x[edges[1]] #Coordinates of the ending vertices of the edges

        Theta_ij=theta_x[edges[0],edges[1]] #Extract theta_x(i,j)
        
        M2=(X_i-x_j)*self.w_coord(Theta_ij.unsqueeze(-1)) #(x_i-x_j)*theta_x(i,j)
       
        M=M1+M2 #M(i,j)=(h_i-h_j)*theta_f(i,j) + (x_i-x_j)*theta_x(i,j)
    ...
    def Compute_Aggregated_Angle(self,x:torch.tensor,A:torch.sparse.Tensor):
        ...
        theta_ij = torch.asin(dot_products) 
    ...
    def forward(self,input:torch.tensor,A:torch.sparse.Tensor,x:torch.Tensor,batch:torch.tensor=None,dropout:float=None) -> torch.tensor:
        ...
        for i in range(self.L):
           ...
           m=self.Compute_Message_Vector(A,input,coord,theta_f,theta_x) #Get the messsage vector for individual edges
