...
def train(model, device, loader, optimizer, task_type):
    ...
    for step, batch in enumerate(loader):
        ...
        batch = batch.to(device)   #Extract the src and est nodes of the edges

        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            pass
        
        else:
            input=batch.x.to(device,dtype=torch.float32)  

            indices=batch.edge_index.to(device,dtype=torch.float32)

            num_nodes=batch.num_nodes

            A=Get_sparse_adjacency_matrix(indices,num_nodes,device) #Get the sparse adjacency matrix
        
            x=torch.rand(input.shape[0],args.hidden,device=device,dtype=torch.float32) #Initialize the coordinates
            ...
            graph_level_logits=global_mean_pool(pred,batch.batch).squeeze(-1)
            
            if "classification" == task_type:
                loss = cls_criterion(graph_level_logits.to(torch.float32)[is_labeled], batch.y.to(torch.float32)[is_labeled])
            else:
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1:
            ...
            with torch.no_grad():
                ...
                graph_level_logits=global_mean_pool(pred,batch.batch)
                
                y_true.append(batch.y.view(graph_level_logits.shape).detach().cpu())
